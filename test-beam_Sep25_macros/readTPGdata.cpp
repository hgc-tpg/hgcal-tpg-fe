	...
	if((tsh->channelId()%2)==0) { // RX only
	  ...
	  if(emp_chan==100){
		...
		std::cout << "Word " << std::setw(6) << wpspd++ << " = 0x"
	      ...
	      for(unsigned el(0);el<7;el++) {
		std::cout << "\t elink " << std::setw(3) << el << " = 0x"
			  << std::hex << std::setfill('0')
			  << std::setw(8) << elinks[el]
			  << std::dec << std::setfill(' ')
			  << std::endl;	      
	      }
	      // TPGFEDataformat::TcRawDataPacket rdp;
	      // TPGStage1Emulation::Stage1IO::convertElinksToTcRawData(TPGFEDataformat::BestC, 23, elinks, rdp);
	      // rdp.print();	    
	    }
	  }
	}
